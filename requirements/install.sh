...
SUPPORTED_ENVS=("behavior" "maniskill_libero" "metaworld" "calvin" "isaaclab" "robocasa" "franka" "frankasim" "robotwin" "habitat" "opensora" "cosmos")
...
install_openpi_model() {
    case "$ENV_NAME" in
        ...
        opensora)
            ...
            install_flash_attn
            install_opensora_world_model
            # Ensure correct transformers version for openpi (opensora.txt may downgrade it)
        ...
        cosmos)
            ...
            uv pip install git+${GITHUB_PREFIX}https://github.com/RLinf/openpi
            install_cosmos_world_model
            # Ensure correct transformers version for openpi
            uv pip install transformers==4.53.2
            ;;
        robotwin)
            create_and_sync_venv
...
install_cosmos_world_model() {
    # Clone Cosmos Predict 2 repository and install from source.
    # The PyPI package has a broken hydra-core/omegaconf pin conflict,
    # so we install the code with --no-deps and add required runtime
    # dependencies via a separate requirements file.
    local cosmos_dir
    cosmos_dir=$(clone_or_reuse_repo COSMOS_PATH "$VENV_DIR/cosmos-predict2" ${GITHUB_PREFIX}https://github.com/nvidia-cosmos/cosmos-predict2.git)
    uv pip install -e "$cosmos_dir" --no-deps

    # Install Cosmos runtime dependencies (heavy NVIDIA wheels from custom index)
    local cosmos_index="https://nvidia-cosmos.github.io/cosmos-dependencies/cu126_torch260/simple"
    uv pip install -r "$SCRIPT_DIR/embodied/models/cosmos.txt" --extra-index-url "$cosmos_index"

    # flash-attn is needed by openvla-oft (prismatic). Pin <2.8 for torch 2.6 ABI compat.
    uv pip install "flash-attn>=2.6,<2.8" --no-build-isolation || echo "WARNING: flash-attn install failed, continuing..."

    # apex is only used for Cosmos training (FusedAdam optimizer), not inference.
    # Its install often fails due to CUDA version mismatches, so we allow it to fail.
    install_apex || echo "WARNING: apex install failed (OK for inference-only usage)."

    # Bake pip-installed NVIDIA CUDA 12.x libs into LD_LIBRARY_PATH on activation
    # (needed when system CUDA < 12 but pip torch/TE ship CUDA 12.x .so files)
    local nvidia_lib_dir="$VENV_DIR/lib/python${PYTHON_VERSION}/site-packages/nvidia"
    if [ -d "$nvidia_lib_dir" ]; then
        cat >> "$VENV_DIR/bin/activate" <<'NVIDIA_LD_EOF'

# Auto-set LD_LIBRARY_PATH for pip-installed NVIDIA CUDA 12.x libraries
_NVIDIA_LIB_DIR="$VIRTUAL_ENV/lib/python${PYTHON_VERSION:-3.10}/site-packages/nvidia"
if [ -d "$_NVIDIA_LIB_DIR" ]; then
    _NVIDIA_PATHS=""
    for _pkg_lib in "$_NVIDIA_LIB_DIR"/*/lib; do
        [ -d "$_pkg_lib" ] && _NVIDIA_PATHS="${_NVIDIA_PATHS:+${_NVIDIA_PATHS}:}$_pkg_lib"
    done
    export LD_LIBRARY_PATH="${_NVIDIA_PATHS}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
    unset _NVIDIA_PATHS _pkg_lib
fi
unset _NVIDIA_LIB_DIR
NVIDIA_LD_EOF
        # Replace the placeholder with the actual Python version
        sed -i "s|\${PYTHON_VERSION:-3.10}|${PYTHON_VERSION}|g" "$VENV_DIR/bin/activate"
    fi
}
