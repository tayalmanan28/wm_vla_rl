...
class OpenPi0ForRLActionPrediction(PI0Pytorch, BasePolicy):
    ...
    def obs_processor(self, env_obs):
        ...
        if env_obs["wrist_images"] is not None:
            ...
            # Provide dummy wrist image (zeros) when not available (e.g., world model envs)
            # Shape matches main_images: [batch, H, W, 3]
            processed_obs["observation/wrist_image"] = torch.zeros_like(env_obs["main_images"])
        # store used keys
